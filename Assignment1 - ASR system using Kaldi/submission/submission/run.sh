...
# Stage 5: Data perturbance
if [ $stage -le 5 ]; then
  ### Triphone
    utils/data/perturb_data_dir_speed_3way.sh data/train data/train_sp3

    echo "Perturbed Triphone training"
    steps/align_si.sh --nj $nj --cmd "$train_cmd" \
       data/train data/lang exp/mono exp/mono_ali
  steps/train_deltas.sh --boost-silence 1.5  --cmd "$train_cmd"  \
     3000 30000 data/train data/lang exp/mono_ali exp/tri1_sp3
    echo "Perturbed Triphone training done"
  #Add triphone decoding steps here #
  (
    echo "Decoding the test set"
    ...
    echo "Perturbed Triphone decoding done."
    ) &
fi

if [ $stage -le 6 ]; then
  ### Triphone
  local/prepare_data.sh train truetest
  for x in train truetest; do
      steps/make_mfcc.sh --nj 20 --cmd "$train_cmd" data/$x exp/make_mfcc/$x mfcc
      steps/compute_cmvn_stats.sh data/$x exp/make_mfcc/$x mfcc
   done
    ...
    steps/align_si.sh --nj $nj --cmd "$train_cmd" \
       data/train data/lang exp/mono exp/mono_ali
  steps/train_deltas.sh --boost-silence 1.5  --cmd "$train_cmd"  \
     3000 30000 data/train data/lang exp/mono_ali exp/tri1_task8
    echo "Perturbed Triphone training done"
  #Add triphone decoding steps here #
  (
    echo "Decoding the test set"
    utils/mkgraph.sh data/lang exp/tri1_task8 exp/tri1_task8/graph
  
    # This decode command will need to be modified when you 
    # want to use tied-state triphone models 
    steps/decode.sh --nj $test_nj --cmd "$decode_cmd" \
      exp/tri1_task8/graph data/truetest exp/tri1_task8/decode_test
    ) &
fi
